...
public class DatabaseTableInfo {
	...
	private static Map<String, DatabaseTableInfo> databaseTableInfoMap = new HashMap<String, DatabaseTableInfo>();
...
//		ResultSet primaryKeySet = this.dmd.getPrimaryKeys(null, null, this.tableName.toUpperCase());
		ResultSet primaryKeySet = this.dmd.getPrimaryKeys(null, null, this.tableName.toLowerCase());
		...
		List<ColumnProperties> columnPropertiesList = new ArrayList<ColumnProperties>(); // 所有字段
		// 初始化主键类型
		...
		// 初始化PO类
		po = this.createBeanFile(className + "Po", Config.poPackage, Config.domainModuleName, columnPropertiesList, null, 0);

		// 初始化condition类
		condition = this.createQueryBean(className + "Condition", Config.conditionPackage, Config.domainModuleName, false);

		// 初始化queryForm类
		queryForm = this.createQueryBean(className + "QueryForm", Config.formPackage, Config.webModuleName, true);

		// 初始化createForm
		createForm = this.createInsertFrom(className + "CreateForm", Config.formPackage, Config.webModuleName);

		// 初始化updateForm
		updateForm = this.createUpdateFrom(className + "UpdateForm", Config.formPackage, Config.webModuleName);

		// 初始化vo
		vo = this.createBeanFile(className + "Vo", Config.voPackage, Config.webModuleName, columnPropertiesList, null, 3);
	...
	public ColumnProperties filterColumn(String fieldId) {
		for (ColumnProperties columnProperties : po.getPropertiesList()) {
			if (columnProperties.getFieldId().equals(fieldId)) {
				return columnProperties;
			}
		}
		return null;
	...
	private void initTableComment() throws SQLException {
		...
		if (JDBC.dbType == 1) {
			...
			commentSql = "select comments from all_tab_comments where table_name = '" + tableName.toUpperCase() + "'";
		} else {
			throw new SQLException("没有对应的db方式");
		}
		Statement commentSt = conn.createStatement();
	...
	private BeanFile createQueryBean(String className, String packageName, String moduleName, boolean isForm) {
		...
		List<String> defaultImportList = new ArrayList<String>();

		List<ColumnProperties> conditionPropertiesList = new ArrayList<ColumnProperties>();
		for (ColumnProperties columnProperties : po.getPropertiesList()) {
			if (columnProperties.getClazz().equals("Date")) {
				if (betweenField.contains(columnProperties.getFieldId())) {
					ColumnProperties minProperties = copyObject(columnProperties);
					...
					ColumnProperties maxProperties = copyObject(columnProperties);
				...
				if (listField.contains(columnProperties.getFieldId())) {
					ColumnProperties listProperties = copyObject(columnProperties);
	 ...
	 * @return
     */
	private ColumnProperties copyObject(ColumnProperties columnProperties) {
		ColumnProperties properties = new ColumnProperties();
		properties.setPrimary(columnProperties.isPrimary());
		properties.setClazz(columnProperties.getClazz());
		properties.setFieldId(columnProperties.getFieldId());
		properties.setComment(columnProperties.getComment());
		properties.setName(columnProperties.getName());
		return properties;
	}
	...
	private BeanFile createInsertFrom(String className, String packageName, String moduleName) {
		...
		List<ColumnProperties> formPropertiesList = new ArrayList<ColumnProperties>();
		for (ColumnProperties columnProperties : po.getPropertiesList()) {
